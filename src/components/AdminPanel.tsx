...
export function AdminPanel() {
  ...
  const handleSave = async () => {
    try {
      ...
      if (cleanData.order !== undefined && cleanData.order !== "") {
        cleanData.order = Number(cleanData.order);
      }

      if (isEditing === "new") {
        if (cleanData.order === undefined || isNaN(cleanData.order)) {
          const siblings = products.filter((p) => p.category === cleanData.category);
          cleanData.order = (siblings.length + 1) * 10;
        }
  ...
  const reorderAllInCategory = async (categoryName: string) => {
    if (!categoryName || categoryName === "Tümü" || categoryName === "Tükenenler") return;
    ...
    try {
      ...
      const writes = siblings.map((item, idx) => {
      ...
      console.warn("Kategori sıralama eşitleme hatası:", e);
      handleFirestoreError(e, OperationType.WRITE, "products", false);
    } finally {
            ...
            <div className="bg-[#16191E] rounded-3xl md:rounded-[40px] border border-white/5 overflow-hidden shadow-2xl">
              ...
              <div className="p-4 md:p-6 border-b border-white/5 flex flex-col md:flex-row gap-4 items-center justify-between bg-white/[0.01]">
                ...
                <div className="flex items-center gap-2 w-full md:w-auto">
                  {selectedCategoryFilter !== "Tümü" && selectedCategoryFilter !== "Tükenenler" && (
                    <button
                      type="button"
                      onClick={() => reorderAllInCategory(selectedCategoryFilter)}
                      disabled={isReordering}
                      className="px-3.5 py-2.5 bg-bamm-yellow/10 hover:bg-bamm-yellow/20 text-bamm-yellow border border-bamm-yellow/20 rounded-xl md:rounded-2xl text-[11px] font-black uppercase tracking-wider flex items-center gap-2 cursor-pointer transition-all shrink-0 active:scale-95 disabled:opacity-50"
                      title="Bu kategorideki tüm ürünlerin sıralamasını 10, 20, 30... olarak veritabanına kaydeder"
                    >
                      <ArrowUpDown size={13} />
